...
#         self.dense4 = nn.Linear(250, 136)
        ...
        self.batchNorm1d3 = nn.BatchNorm2d(128)
...
#         x = self.dense4(x)
        ...
        x = self.batchNorm1d1(self.pool(F.relu(self.conv1(x))))
        x = self.dropout1(x)
        x = self.batchNorm1d2(self.pool(F.relu(self.conv2(x))))
        x = self.dropout2(x)
        x = self.batchNorm1d3(self.pool(F.relu(self.conv3(x))))
        x = self.dropout3(x)
